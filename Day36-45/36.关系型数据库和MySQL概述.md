...
5. 关系数据库产品。
   ...
   - [SQLite](https://www.sqlite.org/) - SQLite是一个c语言库，它实现了一个小型、快速、自包含、高可靠性、全功能的SQL数据库引擎。SQLite是世界上使用最多的数据库引擎。SQLite内置在所有手机和大多数电脑中，并捆绑在人们每天使用的无数其他应用程序中
...
##### 通过Brew 安装的mysql
1. 启动 brew services  start mysql
2. 关闭 brew services  stop mysql
3. 重启 brew services  restart mysql
